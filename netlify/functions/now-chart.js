...
(() => {
  ...
  function normalizeRows(data){
    const out = (data || []).map(r=>{
      ...
      return {
        t: new Date(tt),
        _t: tt,
        fx:   (r.usdkrw_spot != null && r.usdkrw_spot !== "") ? Number(r.usdkrw_spot) : null,
        kr10: (r.kor_10y     != null && r.kor_10y     !== "") ? Number(r.kor_10y)     : null,
        us10: (r.us_10y      != null && r.us_10y      !== "") ? Number(r.us_10y)      : null,
        sofr: (r.sofr_30d    != null && r.sofr_30d    !== "") ? Number(r.sofr_30d)    : null,
      };
    }).filter(Boolean);
  ...
  function trimToMostRecent3Y(rows){
    ...
    const end = rows[rows.length-1]._t; // rows는 asc
  ...
  function drawChart(){
    ...
    const fxVals = chartEnabled.fx ? chartCache.map(d=>d.fx).filter(v=>v!=null) : [];
    const rateVals = []
      .concat(chartEnabled.kr10 ? chartCache.map(d=>d.kr10).filter(v=>v!=null) : [])
      .concat(chartEnabled.us10 ? chartCache.map(d=>d.us10).filter(v=>v!=null) : [])
      .concat(chartEnabled.sofr ? chartCache.map(d=>d.sofr).filter(v=>v!=null) : []);
    ...
    [fxMin, fxMax] = padRange(fxMin, fxMax, 0.06);
    [rMin,  rMax ] = padRange(rMin,  rMax,  0.12);
  ...
  function attachTooltipOnce(){
    ...
    wrap.addEventListener("mousemove", (e)=>{
      ...
      const html = \`
        <span class="tdate">\${formatDateLabel(new Date(d._t))}</span>
  ...
  async function load(){
    ...
    try{
      // ✅ 핵심 수정:
      // - 최신 데이터부터 가져오기 위해 DESC
      // - 3년치(일별)면 대략 1100행 정도라 2000이면 충분
      const resp = await sb
        .from(REL)
      ...
      // DESC로 받은 뒤 → normalizeRows가 ASC로 정렬함
